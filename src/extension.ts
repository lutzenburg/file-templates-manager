...
export async function activate({ extensionPath, globalStoragePath }: ExtensionContext) {
  const templatesManager = await createTemplatesManager(extensionPath, globalStoragePath);
